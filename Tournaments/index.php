...
<div id="main">
	<div class="content">
<?php
	$tournament_name = $_GET['id'];	 //the product id from the URL 
	...
	if ($teammates != 1)
	{
	//---------------------------------Get Free_Agents from Database-----------------------------------------
		$query = 'SELECT gamertag, contact, description FROM DB_1.free_agents WHERE tournament = $1 ORDER BY gamertag';
		$stmt = pg_prepare($conn, 'free_agents', $query) or die("prepare failed: ".pg_last_error() );
		$result = pg_execute($conn, 'free_agents', array($tournament_name) ) or die ("execute failed: ".pg_last_error() );
		$numRows = pg_num_rows($result);
		//If No one has registered for the Event
		if($numRows == 0 )
		{
			echo "\n</br><div class ='alert alert-warning alert-dismissable'>";
			echo'<center>No Free Agents have registered for this Tournament.</center>';
			echo "\n</div>";
		}
		else
		{
			//Display Table of Free Agents
			echo "\n</br><center><div class='panel panel-success'><div class='panel-footer'><h5>Free Agents</h5></div></div></center>";
	...
	else //Else it's a 1v1 or FFA no free agents needed...waiting on admin_brackets.php to be perfected
	{
